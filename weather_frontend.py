...
class WeatherApp:
    ...
    def fetch_weather(self):
        ...
        thread = threading.Thread(target=self._api_call, args=(city,), daemon=True)
        thread.start()

    def _api_call(self, city):
        """Make API call to backend"""
        try:
            # Backend API running on API_URL
            ...
            if response.status_code == 200:
                self._display_result(response.json())
            ...
            self.status_label.config(text="Request timed out", foreground="red")
            messagebox.showerror("Timeout", "Request timed out")
        except Exception:
            self.status_label.config(text="An unexpected error occurred", foreground="red")
